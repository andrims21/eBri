...
use Andriynto\Api\Api;
use Andriynto\Api\Facades\Request;
use Andriynto\Api\OAuth2\OAuthServer;
...
class ApiServiceProvider extends ServiceProvider
{
	...
	{
		$this->package('andriynto/api', 'andriynto/api');
    ...
    {
        $this->app->bind('League\OAuth2\Server\Storage\ClientInterface', 'Andriynto\Api\OAuth2\Repositories\FluentClient');
        $this->app->bind('League\OAuth2\Server\Storage\ScopeInterface', 'Andriynto\Api\OAuth2\Repositories\FluentScope');
        $this->app->bind('League\OAuth2\Server\Storage\SessionInterface', 'Andriynto\Api\OAuth2\Repositories\FluentSession');
        $this->app->bind('Andriynto\Api\OAuth2\Repositories\SessionManagementInterface', 'Andriynto\Api\OAuth2\Repositories\FluentSession');

        $this->app['api.authorization'] = $this->app->share(function ($app) {

            $server = $app->make('League\OAuth2\Server\Authorization');

            $config = $app['config']->get('andriynto/api::oauth2');
        ...
        $this->app['api.resource'] = $this->app->share(function ($app) {
            ...
            return $app->make('Andriynto\Api\OAuth2\Resource');
    ...
    public function registerApi()
    {
        $this->app['api'] = $this->app->share(function ($app) {
            $config = $app['config']->get('andriynto/api::config');
            $config['oauth2'] = $app['config']->get('andriynto/api::oauth2');
